...
先锁定有限交付契约：本次目标、**预期成品清单**、必要审核、目标文件夹、过程目录和停止条件；不用全套台账。有文件交付时在受控过程目录保存 `delivery-contract.json`，按 `scripts/delivery_gate.py --contract 路径` 核验；脚本只检查声明与实际文件，只有已被宿主信任并执行的钩子才能要求返工。已启用 Codex 钩子时，按开工注入的 session_id 调用 `scripts/codex_delivery_hook.py --register --session-id ID --contract 绝对路径`；纯分析用 `--analysis-only --session-id ID`，不得把文件交付声明成无文件。未启用时仍主动运行 gate，不宣称存在自动阻断。

- 每次任务开工先给一段**简短开工小结**，至少包含：初判 T1—T10、当前目标、预期交付物、目标文件夹／交付位置、关键约束和停止条件。材料尚未读取时可标“初判”，后续仅在实质变化时更新。
- 不编造未来耗时承诺。若当前宿主能取得可靠任务起点，可记录实际计时口径；无法取得可靠起止时间时，结尾明确“无法精确核实总耗时”，不得伪造精确秒数。
- 用户纠偏、补救与继续同一任务沿用原任务，不把每条消息当新任务；范围实质变化时更新交付契约和开工小结，不偷偷扩展。

1. **T0**：锁定物件、动作、约束、依据、交付位置、当前阶段和停止条件。有源文件先归档。
2. **轻改直接完成**：一句、一段、已知字段的局部修改，或无新增事实／高风险要求的简单单文件转换与排版，通常 T1-T2；保真、原稿和必要核验不省略，但不因文件后缀启动多轮规划、双技能或全文重复复核。
...
5. **总审**：按 [Review](review-system/review-system.md) 验用户要求、遗漏、越界、真实文件和目录清单。平台已完成且可复用的技术校验不重复跑；Cabinet 只补尚未覆盖的业务、中文办公和最终验收。T4+ 独立审核，T7+ 至少两条独立判断路径；T9+ 按阶段复核，不凑固定执行人数。

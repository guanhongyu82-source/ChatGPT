...
仅当用户在当前请求中明确要求“使用／交给 Sol Cabinet”“SC处理”或调用 `$sol-cabinet` 办理**新任务**时，使用本入口。普通 Codex／Work 消息、仅讨论 Cabinet、同一任务的纠偏与“开始”“继续”“确认”等短回复不触发启动卡。T1-T10 仍负责原有审核和安全门槛；以下四档只表示本次工作规模与粗略耗时，不决定 Agent 人数，也不是硬预算。

根据用户描述和可见文件元数据，综合输入数量与规模、步骤、外部事实、多源核验、交付物依赖和风险，**按命中的最高档**内部判断；微小的已知位置编辑不因整个文件较大单独升档。临界情形可用 `scripts/classify_task.py` 的 `start_tier` 辅助结果，主 Agent 仍按实际语义裁决，不让用户填画像。

| 启动档 | 判定线索 | 粗略预计 |
|---|---|---|
| 快速 | 单一输入／交付、至多两步，无外部事实核验、复杂依赖或高风险 | 约 5–10 分钟 |
| 标准 | 少量输入、三至五步、一次有限核验或两个简单交付物 | 约 10–25 分钟 |
| 复杂 | 多源综合、六步以上、多个相关交付物、复杂版式、独立复核或明显风险 | 约 25–60 分钟 |
| 分阶段 | 大批量、十步以上、长期运行或多个需单独授权的里程碑 | 按当前阶段另估 |

若宿主提供原生点选控件，显示**一次**简短卡片：任务、预计交付、启动档、粗略耗时、推荐路径；给出“按推荐方案开始／调整交付／取消”三个选项。只把当前卡片的点选回执当作启动选择，不解析普通文本为确认。`调整交付` 只显示本任务可行的点选方案，选定后更新卡片，再由用户点“开始”；不要求键入 A/B/C/D 或其他文字。点击取消即停止。选择开始后归档已有原稿，再读正文并按既有交付规则执行；执行中纠偏沿用当前任务，不重出卡。已知保密、权限或材料硬阻断仍按原规则处理，不以点选放行。

宿主没有可用的点选控件时，沿用 v1.5.5 的直接执行路径，不用纯文本假按钮要求用户输入，不新增全局 Hook、预算熔断或任务状态机。预计耗时不是完成保证或硬截止。

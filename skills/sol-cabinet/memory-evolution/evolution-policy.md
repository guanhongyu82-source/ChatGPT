...
## Evolution Checkpoint：每次收尾必经，命中才进入维护

在最终完工小结前，主 Agent 必须且只执行一次轻量 **Evolution Checkpoint**。这是收尾分类，不是维护流程本身，目标是消除“只有模型主动想起失败才进入 Evolution”的隐式触发。

Checkpoint 只能使用本任务已经发生且主 Agent 已知的事实，不得为了判断而重新读取文件、重复渲染／校验、扫描 observations／proposals、调用 `status`、跑回归或创建证据。

只检查以下已知信号：用户纠正、漏文件／对象／步骤、假完成、质量失败、误路由、多文件遗漏、角色未执行、不必要串行、重复读取／检查、规则未生效、运行时兼容、质量退化，以及已经明确发生的任务低估级或交付失控。普通审美偏好、正常润色、用户新增要求、用户改变目标和外部事实变化不算 Cabinet 事故。

Checkpoint 只有四种可见状态：

- `CLEAN`：未发现需记录的真实系统问题。立即结束进化路径；零账本读取、零 incident、零回归、零 EVO。
- `RECORDED`：发现真实问题，完成当前返工后已通过现有 `record` 安全写入。只表示入队，不表示已修复或已晋升。
- `PENDING`：发现真实问题，但当前环境、证据或安全门禁不允许完成记录。必须明确“未记录／未同步”，不得声称后台稍后自动完成。
- `EVO-...`：仅真实正式晋升成功后使用，沿用既有 EVO 语义；Checkpoint 本身不得生成 EVO。

发现信号时才读取本文件后续维护规则、准备脱敏证据并调用现有 `record`。若 `record` 返回拒绝或写入失败，本次状态只能是 `PENDING`，不得递归制造新的自我进化事故，也不得伪装为 `RECORDED`。

完工小结必须用一行显示本次 Checkpoint 状态，使用户能够确认自进化入口是否实际执行；除这一行外不新增“反思报告”或长篇进化说明，除非用户明确要求。

## 生产收尾：无事故零写入

生产 Agent 仅执行办公、解决当前返工、发现并提供证据，不改 Skill。Evolution Checkpoint 统一判断：用户纠正、漏文件/对象/步骤、假完成、质量失败、误路由、多文件遗漏、角色未执行、不必要串行、重复读取检查、规则未生效、兼容与质量退化。普通审美、措辞偏好、正常润色不当事故。
...
LAB.4 起，每次完工小结均保留一行 Evolution Checkpoint 状态，具体格式见 [Delivery Summary](../templates/delivery-summary.md)。正式 EVO 仍只在真实晋升成功时使用“进化：EVO-YYYYMMDD-HHMMSS｜改进内容｜质量↑ / 速度↑｜回归通过”；只填实际有证据的提升维度。`CLEAN`、`RECORDED`、`PENDING` 均不得冒充 EVO，也不新增日志块。

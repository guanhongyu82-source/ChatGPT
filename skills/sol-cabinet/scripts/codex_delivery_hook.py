...
def handle(event,state_root=STATE,allowed_root=BASE):
    ...
    if kind=='UserPromptSubmit':
        ...
        return {'hookSpecificOutput':{'hookEventName':kind,'additionalContext':
          'Sol Cabinet 执行提醒：开工先给简短小结，包含T级、当前目标、预期交付物、目标文件夹/位置、关键约束和停止条件；不要编造未来耗时承诺。沿用当前模型，不自行换型。文件任务先识别既有Task ID和Task Root，跨日、补充材料或Reopen不得新建任务夹；Task Card的deliverables锁定唯一Expected清单，每项至少写artifact_id、required、format、target_role、target_directory，用户指定文件名才写filename_override；不得另建第二份Expected。新增材料用现有原稿归档机制自动登记批次，hash重复不复制，同名异内容不覆盖，未知用途阻断归档。最终目录可保留本任务历史正式版本和当前正式版本，但不得混入过程稿、审核 JSON、日志、测试件、缓存或临时转换件。交付前生成delivery-contract.json：task_card只记录Task Card绝对路径和当前sha256，artifacts只记录Actual并以artifact_id对应Expected；用户改变交付要求时先更新Task Card再重新登记契约。正式归档须先核Task Root Archive，再核原稿、work、outputs、版本连续性、材料追溯和11项FINAL PASS门槛；根目录临时名或未知文件时禁止FINAL PASS。然后调用 scripts/codex_delivery_hook.py --register --session-id '+event['session_id']+' --contract 绝对路径。Stop会校验Task Card锁、Expected↔Actual、目录与完工小结；收尾必须列实际成品及路径、目录状态、核验、未完成项和真实耗时/不可核实原因。有失误说明处置并按evolution-policy记录。无文件的分析答疑不虚造Task Card成品或文件契约，调用同脚本 --analysis-only --session-id '+event['session_id']+' 明确无文件交付。同一激活内file/analysis模式一经声明即锁定，不得由执行端自行切换；需要改变交付模式必须重新显式触发Sol Cabinet。首次Stop失败只允许一次定向返工；再次失败进入TERMINAL_PARTIAL并停止自动重试。终态不会被普通后续消息复活，若确需重新进入钩子治理须再次显式触发Sol Cabinet。'}}

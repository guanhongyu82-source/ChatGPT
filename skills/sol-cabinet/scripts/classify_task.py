...
def classify_start_tier(profile: dict[str, Any]) -> dict[str, Any]:
    """Estimate explicit Cabinet launch size; never gate ordinary prompts or tools."""
    ...
    if complexity > 3:
        raise ValueError("complexity must be in 0..3")
    bounded = _bounded_edit(profile)

    if (files >= 20 or (size_kb >= 100_000 and not bounded)
            or deliverables >= 4 or steps >= 10
            or _flag(profile, "multi_stage") or _flag(profile, "long_running")):
        tier = 4
    elif (files >= 5 or (size_kb >= 10_000 and not bounded)
          or deliverables >= 3 or steps >= 6 or complexity >= 2
          or any(_flag(profile, key) for key in (
              "high_risk", "regulated", "secret_bearing", "destructive_action",
              "formal_normative_additions", "system_build", "needs_review"))
          or (_flag(profile, "multi_source")
              and (_flag(profile, "needs_research") or _flag(profile, "fact_risk")))):
        tier = 3
    elif (files >= 2 or (size_kb >= 1_000 and not bounded)
          or deliverables >= 2 or steps >= 3 or complexity >= 1
          or any(_flag(profile, key) for key in (
              "needs_research", "current_fact", "fact_risk", "multi_source",
              "formal_publish", "external_action", "sensitive"))):
        tier = 2
    else:
        tier = 1
    labels = ("快速", "标准", "复杂", "分阶段")
    estimates = ((5, 10), (10, 25), (25, 60), (60, None))
    return {"tier": tier, "label": labels[tier - 1],
            "estimate_minutes": list(estimates[tier - 1]),
            "estimate_only": True, "hard_budget": False}
...
def classify(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "start_tier": classify_start_tier(profile),
        "t_level": level,
        "score": score,
        "score_axes": axes,

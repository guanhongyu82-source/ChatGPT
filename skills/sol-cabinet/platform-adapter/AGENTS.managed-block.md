...
- 普通任务内部完成轻量 T0，必要时简要告知 T 级与预计耗时；仅用户当前请求显式调用 Sol Cabinet 的新任务，才按 Skill 的 T1-T10 判级并显示简短开工卡。普通消息和“开始”“继续”“确认”不单独触发新任务；已激活任务的确认沿用当前状态。纠偏沿用当前任务；收尾核成果、路径、自检、实际耗时，有实质失误说明处置并记录脱敏事故。
...
- T1-T2 普通任务直接完成，不加载重流程；用户显式调用 Sol Cabinet 办理新任务时加载 `$sol-cabinet` 并先显示开工卡；明确预授权的任务登记后执行，其余等待自然语言确认。正式、多文件、事实敏感、高风险、系统建设或长周期任务仍按需加载技能；仅提及 Cabinet 的讨论不因此启动卡片。

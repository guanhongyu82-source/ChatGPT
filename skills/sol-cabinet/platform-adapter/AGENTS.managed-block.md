...
- Sol Cabinet 每项正式任务开工前先给极简启动卡：T1-T4、预计耗时、预计交付、推荐路径、执行预算及 A/B/C/D。用户未明确选 A 不得正式执行；B/C 重出卡，D 停止。原稿归档在确认后、读正文前进行；纠偏沿用原预算，预算到期停止并报 PARTIAL。
...
- 确认后 T1 由主代理走快车道，不默认启用 Agent、联网或深研；T2-T4 依启动卡预算执行，T4 分阶段并在阶段完成后停下；显式提到 Sol Cabinet／使用 Sol Cabinet／交给 Sol Cabinet／SC处理，或任务为正式、多文件、事实敏感、高风险、系统建设、长周期时，加载 `$sol-cabinet`。

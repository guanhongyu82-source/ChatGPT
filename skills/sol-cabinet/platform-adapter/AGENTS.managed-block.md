...
- 内部完成轻量 T0，先用一句话告知T级与预计耗时，不展示内部分析。纠偏继续沿用原计时；收尾核成果、路径、自检、实际耗时，有实质失误说明处置并记录脱敏事故。
...
- T1-T2 直接完成，不加载重流程；显式提到 Sol Cabinet／使用 Sol Cabinet／交给 Sol Cabinet／SC处理，或任务为正式、多文件、事实敏感、高风险、系统建设、长周期时，加载 `$sol-cabinet`。

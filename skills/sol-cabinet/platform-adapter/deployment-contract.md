...
2. GitHub 中存在对应已验证的 **GITHUB_STABLE** commit；
3. 本次明确要更新 Mac 运行环境，或运行环境必须更新才能使用新版本；
4. 部署目标、源 commit、源系统摘要和回滚点已明确。

仅审阅、提方案、比较、候选测试通过或查看状态不得触发部署。Candidate 默认禁止部署为正式 Runtime；用户明确要求临时候选试装时必须单独标识，不得覆盖 Stable 部署身份或冒充正式更新。

## 标准部署闭环

1. **READ**：读取 `main` 目标 commit、`VERSION` 与本机当前 Runtime 状态。
2. **SOURCE DIGEST**：对选定 GitHub commit 的 `skills/sol-cabinet/` 计算系统摘要，记录 commit + version + digest；不得从 Runtime 反推源身份。

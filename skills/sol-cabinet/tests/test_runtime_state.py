...
class RuntimeStateTests(unittest.TestCase):
    ...
    def valid_lab_record(self, commit='a' * 40, digest='b' * 64, version='1.5.5-lab.4'):
        return {
            'schema_version': 2,
            'record_kind': 'runtime-deployment-state',
            'channel': 'lab',
            'source_ref': 'lab/v1.5.5-lab.4',
            'repository': state.REPOSITORY,
            'repository_path': state.REPOSITORY_PATH,
            'source_commit': commit,
            'source_version': version,
    ...
    def test_lab_capture_records_channel_and_source_ref(self):
        ...
        with mock.patch.object(state, '_digest', return_value='b' * 64), \
             mock.patch.object(state, '_version', return_value='1.5.5-lab.4'):
            record = state.build_state(
                'a' * 40, 'b' * 64, ROOT, now=stamp,
                channel='lab', source_ref='lab/v1.5.5-lab.4',
            )
        self.assertEqual(record['schema_version'], 2)
        self.assertEqual(record['channel'], 'lab')
        self.assertEqual(record['source_ref'], 'lab/v1.5.5-lab.4')
        self.assertEqual(record['source_version'], '1.5.5-lab.4')
    ...
    def test_lab_runtime_matches_expected_channel(self):
        ...
        self.assertEqual(result['source_ref'], 'lab/v1.5.5-lab.4')

...
class HookTests(unittest.TestCase):
 ...
 def test_unrelated_prompt_zero_write(self):
  with tempfile.TemporaryDirectory() as d:
   r=Path(d)
   for prompt in ('你好','请帮我修改一句话','开始','继续','确认','谈谈 Sol Cabinet 的启动门'):
    self.assertEqual(hook.handle(self.event(r,prompt=prompt),r/'state',r),{})
    self.assertFalse((r/'state').exists())
   self.assertEqual(hook.handle(self.event(r,hook_event_name='PreToolUse'),r/'state',r),{})
 def test_prompt_creates_explicit_activation_without_storing_content(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   self.assertIn('TERMINAL_PARTIAL',context);self.assertNotIn('启动卡',context);self.assertEqual(state['schema_version'],2)

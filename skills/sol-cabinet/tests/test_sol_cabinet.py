...
class RouterTests(unittest.TestCase):
    ...
    def test_click_start_tier_uses_highest_scope_signal_without_hard_budget(self):
        cases = [
            ({}, 1),
            ({"file_count": 1, "office_artifact": True, "steps": 2}, 1),
            ({"file_count": 3, "steps": 4}, 2),
            ({"file_count": 6, "multi_source": True, "fact_risk": True}, 3),
            ({"multi_stage": True, "steps": 12}, 4),
            ({"file_count": 1, "input_size_kb": 9000, "bounded_edit": True}, 1),
            ({"bounded_edit": True, "high_risk": True}, 3),
        ]
        for profile, expected in cases:
            with self.subTest(profile=profile):
                result = classifier.classify_start_tier(profile)
                self.assertEqual(result["tier"], expected)
                self.assertTrue(result["estimate_only"])
                self.assertFalse(result["hard_budget"])
                self.assertEqual(classifier.classify(profile)["start_tier"], result)
